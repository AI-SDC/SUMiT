...
JJData::JJData(const char *filename)
{
    ...
    line_number++;
    if (fscanf(ifp, "%d\n", &file_id) != 1) {
        // logger->error(202, "Incorrect format at line %d: %s", line_number,
        // filename);
    }
    ...
    line_number++;
    if (fscanf(ifp, "%d\n", &ncells) != 1) {
        // logger->error(204, "Incorrect format at line %d: %s", line_number,
        // filename);
    }
    ...
    for (CellIndex i = 0; i < ncells; i++) {
        ...
        if (fscanf(ifp, " %d %lf %lf %c %lf %lf %lf %lf %lf\n", &cells[i].id,
                   ...
                   &cells[i].loss_of_information_weight, &cells[i].status,
                   &cells[i].lower_bound, &cells[i].upper_bound,
                   &cells[i].lower_protection_level,
                   &cells[i].upper_protection_level,
                   &cells[i].sliding_protection_level) != 9) {
            // logger->error(205, "Incorrect format at line %d: %s",
    ...
    line_number++;
    if (fscanf(ifp, "%d\n", &nsums) != 1) {
        // logger->error(208, "Incorrect format at line %d: %s", line_number,
        // filename);
    }
    ...
    for (SumIndex i = 0; i < nsums; i++) {
        ...
        if (fscanf(ifp, "%lf ", &consistency_eqtns[i].RHS) != 1) {
            // logger->error(209, "Incorrect total format at line %d: %s",
            // line_number, filename);
        }

        if (fscanf(ifp, " %d ", &consistency_eqtns[i].size_of_eqtn) != 1) {
            // logger->error(210, "Incorrect equation size format at line %d:
            // %s", line_number, filename);
        }

        if (consistency_eqtns[i].size_of_eqtn < 2) {
            // logger->error(211, "Equation size too small at line %d: %s",
            // line_number, filename);
        }

        if ((fscanf(ifp, " %c ", &divider) != 1) || (divider != ':')) {
            // logger->error(212, "Incorrect divider format at line %d: %s",
            // line_number, filename);
        }

        consistency_eqtns[i].cell_index =
            new CellIndex[consistency_eqtns[i].size_of_eqtn];
        consistency_eqtns[i].plus_or_minus =
            new int[consistency_eqtns[i].size_of_eqtn];
        consistency_eqtns[i].marginal_index = -1;

        for (CellIndex j = 0; j < consistency_eqtns[i].size_of_eqtn; j++) {
            CellID id;
            if (fscanf(ifp, " %d (%d)", &id,
                       &consistency_eqtns[i].plus_or_minus[j]) != 2) {
                // logger->error(213, "Incorrect consistency equation format at
                // line %d: %s", line_number, filename);
            }
            ...
            if (consistency_eqtns[i].plus_or_minus[j] < 0) {
                if (consistency_eqtns[i].marginal_index == -1) {
                    consistency_eqtns[i].marginal_index =
                        consistency_eqtns[i].cell_index[j];
                } else {
                    // logger->error(222, "Duplicate total in consistency
                    // equation at line %d: %s", line_number, filename);
                }
            }
        }

        if (consistency_eqtns[i].marginal_index == -1) {
            // logger->error(223, "Missing total in consistency equation at line
            // %d: %s", line_number, filename);
        }
    }
    ...
    for (SumIndex i = 0; i < nsums; i++) {
        ...
        for (CellIndex j = 0; j < size; j++) {
            sum = sum +
                (double) consistency_eqtns[i].plus_or_minus[j] *
                    cells[consistency_eqtns[i].cell_index[j]].nominal_value;
        }

        if (fabs(sum - consistency_eqtns[i].RHS) >
            MARGINAL_EQUALITY_TOLERANCE) {
            // logger->log(1, "Inconsistent constraint at line %d: constraint
            // index = %d, sum = %lf, RHS = %lf", line_number, i, sum,
            // consistency_eqtns[i].RHS);
            for (CellIndex j = 0; j < size; j++) {
                // logger->log(1, "Cell index %d, ID %d, value %lf",
                // consistency_eqtns[i].cell_index[j],
                // cell_index_to_id(consistency_eqtns[i].cell_index[j]),
                // (double)consistency_eqtns[i].plus_or_minus[j] *
                // cells[consistency_eqtns[i].cell_index[j]].nominal_value);
            }
            error = true;
    ...
    if (error) {
        // logger->error(214, "JJ file contains one or more inconsistent
        // constraints: %s", filename);
    }
    ...
    do {
        ...
        for (SumIndex i = 0; i < nsums; i++) {
            ...
            // Level of marginal is one greater than maximum level of underlying
            // cells
            int level = 0;

            for (CellIndex j = 0; j < eqtn->size_of_eqtn; j++) {
                if ((eqtn->cell_index[j] != marginal_index) &&
                    (cells[eqtn->cell_index[j]].level > level)) {
            ...
            // Set level of marginal if it is greater than that arising from
...
// This constructor takes care of generating marginals and consistency equations
// for the partition and removes zero cells partition_cells is the set of parent
// cells to include in the partition.  Cells may appear in any order.  Any
// marginals present are ignored. partition_size is the size of the set of
// partition_cells partition_id is a one-based identifier used to identify the
// partition in error and log messages
JJData::JJData(JJData *parent, CellID *partition_cells, int partition_size,
               int partition_id)
{
    ...
    // The partition name is one plus the partition index to match partition
    // naming elsewhere in the code
    ...
    for (int i = 0; i < partition_size; i++) {
        ...
        // Ignore any marginals supplied - we will include any required
        // marginals later on Also ignore protected cells (including those with
        // value zero) as these cannot contribute to cell suppression
        if ((parent->cells[parent_index].level == 0) &&
            (parent->cells[parent_index].status != 'z')) {
            // Values of underlying cells are inherited from parent table and do
            // not change with partitioning
    ...
    for (int level = 1; level < parent->nlevels; level++) {
        ...
        // Add the direct marginals of the current level to the set of required
        // cells
        for (SumIndex i = 0; i < parent->nsums; i++) {
            ...
            if (parent->cells[parent_eqtn->marginal_index].level == level) {
                //                CellID marginal_id =
                //                parent->cell_index_to_id(parent_eqtn->marginal_index);
                if (TRACE(marginal_id)) {
                    //                    // logger->log(5, "Marginal %d: Level
                    //                    %d, value %lf, parent value %lf",
                    //                    marginal_id,
                    //                    parent->cells[parent_eqtn->marginal_index].level,
                    //                    values[parent_eqtn->marginal_index],
                    //                    parent->cells[parent_eqtn->marginal_index].nominal_value);
                }

                // Run through the consistency equation recalculating the
                // marginal using only the cells that are included in the
                // partition
                double value = 0.0;
                for (CellIndex j = 0; j < parent_eqtn->size_of_eqtn; j++) {
                    // Adjust the value of the marginal if the cell is included
                    // in the partition
                    CellIndex parent_index = parent_eqtn->cell_index[j];
                    if (parent_index != parent_eqtn->marginal_index) {
                        // This test is needed to avoid errors accumulating from
                        // adding zero cells that do not have an exact zero
                        // representation
                        if (values[parent_index] >= FLOAT_PRECISION) {
                            ...
                            if (TRACE(marginal_id)) {
                                ...
                                //                                %lf",
                                //                                marginal_id,
                                //                                parent->cell_index_to_id(parent_index),
                                //                                parent->cells[parent_index].level,
                                //                                values[parent_index],
                                //                                parent->cells[parent_index].nominal_value);
                            }
                ...
                // Add the marginal to the set of required cells if it has not
                // already been added and it is non-zero (i.e., required)
                if ((values[parent_eqtn->marginal_index] < FLOAT_PRECISION) &&
                    (value >= FLOAT_PRECISION)) {
                    values[parent_eqtn->marginal_index] = value;
                    if (TRACE(marginal_id)) {
                        //                        // logger->log(5, "Marginal
                        //                        %d: Assigned value %lf",
                        //                        marginal_id,
                        //                        values[parent_eqtn->marginal_index]);
                    }
                } else {
                    if (fabs(value - values[parent_eqtn->marginal_index]) >
                        MARGINAL_EQUALITY_TOLERANCE) {
                        // Marginals generated from different consistency
                        // equations do not match logger->error(215,
                        // "Inconsistent marginal values generated for marginal
                        // %d  in equation index %d (equation value %lf, stored
                        // value %lf)",
                        // parent->cell_index_to_id(parent_eqtn->marginal_index),
                        // i, value, values[parent_eqtn->marginal_index]);
                    }
                }
            }
        }
    ...
    for (SumIndex i = 0; i < parent->nsums; i++) {
        if (generate_partition_consistency_equation(parent, i,
                                                    &partition_eqtn)) {
...
void
JJData::write_jj_file(const char *filename)
{
    FILE *ofp;
    ...
    for (CellIndex i = 0; i < ncells; i++) {
        fprintf(ofp, "%d %f %f %c %f %f %f %f %f\n", cells[i].id,
                cells[i].nominal_value, cells[i].loss_of_information_weight,
                cells[i].status, cells[i].lower_bound, cells[i].upper_bound,
                cells[i].lower_protection_level,
                cells[i].upper_protection_level,
                cells[i].sliding_protection_level);
    }
    ...
    for (SumIndex i = 0; i < nsums; i++) {
        ...
        for (CellIndex j = 0; j < size; j++) {
            fprintf(ofp, " %d (%d)",
                    cell_index_to_id(consistency_eqtns[i].cell_index[j]),
                    consistency_eqtns[i].plus_or_minus[j]);
        }
        fprintf(ofp, "\n");
    }
...
CellIndex
JJData::get_number_of_cells()
{
    return ncells;
}

CellIndex
JJData::get_number_of_primary_cells()
{
    CellIndex count = 0;
...
void
JJData::recombine(const char *partitioned_jj_filename)
{
    ...
    for (CellIndex i = 0; i < partition->ncells; i++) {
        ...
        if (id >= ncells) {
            // logger->error(217, "Cell ID %d out of range for recombination:
            // %s", id, partitioned_jj_filename);
        }
...
bool
JJData::generate_partition_consistency_equation(
    JJData *parent, SumIndex index, ConsistencyEquation *partition_equation)
{
    ConsistencyEquation *parent_eqtn = &parent->consistency_eqtns[index];
    CellIndex marginal_index_in_eqtn =
        parent->find_marginal_index_in_equation(parent_eqtn);
    ...
    for (CellIndex i = 0; i < parent_eqtn->size_of_eqtn; i++) {
        ...
        if ((i == marginal_index_in_eqtn) || (map.count(id) > 0)) {
            ...
            plus_or_minus[partition_equation->size_of_eqtn] =
                parent_eqtn->plus_or_minus[i];
    ...
    if (partition_equation->size_of_eqtn > 1) {
        partition_equation->cell_index =
            new CellIndex[partition_equation->size_of_eqtn];
        partition_equation->plus_or_minus =
            new int[partition_equation->size_of_eqtn];
...
CellIndex
JJData::cell_id_to_index(CellID id)
{
...
CellID
JJData::cell_index_to_id(CellIndex index)
{
    if ((index < 0) || (index >= ncells)) {
...
CellID
JJData::find_marginal_id(ConsistencyEquation *equation)
{
    ...
    if (marginal_index_in_eqtn == -1) {
        // logger->error(220, "Missing marginal in consistency equation: %s",
        // name);
    }
...
CellIndex
JJData::find_marginal_index_in_equation(ConsistencyEquation *equation)
{
    CellIndex marginal_index_in_eqtn = -1;
    ...
    if (marginal_index_in_eqtn == -1) {
        // logger->error(221, "Missing marginal in consistency equation: %s",
        // name);
    }
